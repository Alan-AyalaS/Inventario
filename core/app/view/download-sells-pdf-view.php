...
$pdf->Cell(0, 10, 'Lista de Ventas', 0, 1, 'C');
...
$header = array('ID', 'Cantidad de Productos', 'Total', 'Fecha');
$w = array(20, 50, 50, 70);
...
foreach($sells as $sell) {
    ...
    $pdf->Cell($w[0], 7, $sell->id, 1);
    $pdf->Cell($w[1], 7, count($operations), 1);
    $pdf->Cell($w[2], 7, '$ ' . number_format($total, 2), 1);
    $pdf->Cell($w[3], 7, $sell->created_at, 1);
    $pdf->Ln();
}

// Salida del PDF
$pdf->Output('ventas_' . $current_datetime . '.pdf', 'D');
exit; 